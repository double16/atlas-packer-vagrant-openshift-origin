...
export OSE_VERSION="v1.1"
export OPENSHIFT_IMAGE_REGISTRY="hub.docker.com"
...
if [ ! -f $master_config ]; then
  ...
  sed -i.orig -e "s/\(.*subdomain:\).*/\1 localhost/" ${master_config} \
    -e "s/\(.*masterPublicURL:\).*/\1 https:\/\/localhost:8443/g" \
    -e "s/\(.*publicURL:\).*/\1 https:\/\/localhost:8443\/console\//g" \
    -e "s/\(.*assetPublicURL:\).*/\1 https:\/\/localhost:8443\/console\//g"
...
# FIXME  registry_host_name="hub.$2"
